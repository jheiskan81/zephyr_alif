...
static int hci_alif_open(void)
{
	...
	if (hci_uart_dma_rx_enabled || hci_uart_dma_tx_enabled) {

		ret = uart_callback_set(hci_alif_dev, hci_uart_async_callback, NULL);
	...
	if (!hci_uart_dma_rx_enabled || !hci_uart_dma_rx_enabled) {
		/* Register */
	...
	if (hci_uart_dma_tx_enabled) {
		...
		dma_event_router_configure(DMA_UART_TX_GROUP, tx_config);
	}
